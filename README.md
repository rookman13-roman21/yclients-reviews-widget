...
и текст отзыва. Файл создаётся временно в той же директории, синхронизируется и
заменяется через rename; права `0640`. Секреты yClients, телефоны, e-mail,
полное имя и сырой webhook в feed не попадают.
