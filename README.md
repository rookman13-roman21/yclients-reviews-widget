...
Связь допускается только по явному `client_id` из ответа отзывов yClients;
подбора по имени, телефону или e-mail нет. `client_id` используется лишь в
памяти при построении feed, а KV-кэш привязан к ID отзыва и хранит только дату
и название визита. История не обогащается: при первом запуске функции виджет
