...
- **Контур выпуска:** `reviews-widget.html` собирается в hosted loader и bundle; сначала заменяется bundle, затем loader. Tilda содержит ровно один фрагмент `tilda-embed.html`; при новом cache-buster его нужно заменить целиком и опубликовать страницу.
